  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int l = 1; l < lagNum; ++l) {
      ...
      for (int neighbor : current_neighbors) {
        for (int j = 0; j < n; ++j) {
          if (nbmat[neighbor][j] == 1 && i != j && !visited.count(j)) {
    ...
    result[i] = std::vector<int>(visited.begin(), visited.end());
  }
    ...
    for (size_t j = i; j < n; ++j) {
      if (i == j) {
        ...
        for (size_t k = 0; k < matrix[i].size(); ++k) {
          abs_diff.push_back(std::abs(matrix[i][k] - matrix[j][k]));
        }
