...
We assume an underlying causal interaction structure among species, where species *a* influences *b*, and *b* in turn influences *c* (i.e., *a* → *b* → *c*). The intrinsic growth rates for species *a*, *b*, and *c* are set to 0.55, 0.35, and 0.15 respectively. The effect of species *a* on *b* is set to 0.625, and the effect of species *b* on *c* is set to 0.77. All other interspecific influence parameters are set to zero. This setup provides a controlled environment to test spatial causality detection methods under known dynamic interactions.


``` r
simv = spEDM::slm(species,x = "a", y = "b", z = "c", k = 4, step = 20, transient = 1:3,
                  alpha_x = 0.55, alpha_y = 0.35, alpha_z = 0.15,
...
## source(s)   : memory
## names       :         a,         b,         c 
## min values  : 0.7204142, 0.8690008, 0.9583186 
## max values  : 0.7209665, 0.8700491, 0.9584180
